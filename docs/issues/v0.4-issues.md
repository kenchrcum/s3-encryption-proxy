# Milestone v0.4 — Issue Tracker and Implementation Guide
...
### Implementation Guide
- [ ] Define provider header limits and reserved budget (per RFCs and provider docs).
- [ ] Compact metadata keys/values (short keys, base64url, omit defaults).
- [ ] Add provider profiles and selection via config; warn on nearing limits.
- [ ] Fallback to object metadata storage strategy if headers would overflow (documented).
...
### Definition of Done
- [ ] Profiles for AWS/MinIO/Wasabi/Hetzner documented and enforced
- [ ] No header overflow in integration suite
...
### Implementation Guide
- [ ] Add conformance tests for delimiter/prefix/common prefixes.
- [ ] Support continuation tokens; ensure stable ordering.
- [ ] Translate gateway metadata correctly in listings.
...
### Definition of Done
- [ ] Parity tests pass against at least two providers
...
### Implementation Guide
- [ ] Add robust XML decoding with limits and clear error translation.
- [ ] Fuzz inputs for part ordering, duplicates, invalid ETags.
- [ ] Interop tests: initiate, upload, complete, abort flows across providers.
...
### Definition of Done
- [ ] No panics; correct error mapping; interop green
...
### Implementation Guide
- [ ] Add sized buffer pools (e.g., sync.Pool with buckets).
- [ ] Integrate in hot paths; ensure zeroization on reuse when holding sensitive data.
- [ ] Metrics to gauge hit/miss rates.
...
### Definition of Done
- [ ] Allocation rate drops in benchmarks without regressions
...
### Implementation Guide
- [ ] Review readers/writers for bounded queues.
- [ ] Add context-aware cancellation and timeouts on pipelines.
- [ ] Document recommended limits in `docs/DEPLOYMENT.md`.

### Tests
- Load tests in `test/load_test.go`

### Definition of Done
- [ ] Improved P95 latency and stable memory under load
...
### Implementation Guide
- [ ] Add OTEL SDK wiring with exporter config.
- [ ] Instrument inbound HTTP, backend S3 calls, and crypto steps as spans.
- [ ] Add sampling, redaction of sensitive attributes.
...
### Definition of Done
- [ ] Traces visible end-to-end; cardinality bounded
...
### Implementation Guide
- [ ] Implement presets with structured fields.
- [ ] Redact keys and auth tokens by default.

### Tests
- Unit: format correctness and redaction

### Definition of Done
- [ ] Configurable presets documented in `docs/DEVELOPMENT_GUIDE.md`
...
### Implementation Guide
- [ ] File watcher or SIGHUP handler to reload config.
- [ ] Validate and apply only safe fields; log diffs.
...
### Definition of Done
- [ ] Safe hot-reloads without downtime
